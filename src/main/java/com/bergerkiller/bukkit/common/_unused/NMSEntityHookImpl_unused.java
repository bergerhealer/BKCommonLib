...
public class NMSEntityHookImpl_unused implements NMSEntityHook_unused {

    private EntityController<?> controller;

    public NMSEntityHookImpl_unused(CommonEntity<?> entity) {
    ...
    public boolean damageEntity(DamageSource damageSource, float damage) {
       ...
       //                 .getForHandle(damageSource), damage);
        return true;
    }
    ...
    private String getSavedName() {
        ...
        // return
        // NMSEntityTypes.classToNames.get(controller.getEntity().getHandle().getClass().getSuperclass());
    }
    ...
    public boolean c(NBTTagCompound nbttagcompound) {
        if (this.controller.getEntity().isDead()) {
            ...
            this.controller.getEntity().getHandle(Entity.class)
                    .e(nbttagcompound);
            return true;
        }
    }
    ...
    public boolean d(NBTTagCompound nbttagcompound) {
        if (this.controller.getEntity().isDead()
                || (this.controller.getEntity().hasPlayerPassenger() && controller
                        .isPlayerTakable())) {
            return false;
        } else {
            nbttagcompound.setString("id", getSavedName());
    ...
    public void teleportTo(Location exit, boolean portal) {
        ...
        if (portal) {
            ...
            CommonNMS.getNative(exit.getWorld()).getTravelAgent()
                    .adjustExit(entity.getHandle(Entity.class), exit, velocity);
            if (entity.vel.getX() != velocity.getX()
                    || entity.vel.getY() != velocity.getY()
                    || entity.vel.getZ() != velocity.getZ()) {
                entity.setVelocity(velocity);
            }
        }
        // Finally, teleport it
        this.controller.getEntity().teleport(exit,
                portal ? TeleportCause.NETHER_PORTAL : TeleportCause.UNKNOWN);
    }
    ...
    public boolean super_a(EntityHuman human, ItemStack is, EnumHand hand) {
        return controller.getEntity().getHandle(NMSEntityHook_unused.class)
                .super_a(human, is, hand);
    }

    @Override
    public void super_U() {
        controller.getEntity().getHandle(NMSEntityHook_unused.class).super_U();
    }

    @Override
    public boolean super_damageEntity(DamageSource damagesource, float damage) {
        return controller.getEntity().getHandle(NMSEntityHook_unused.class)
                .super_damageEntity(damagesource, damage);
    }

    @Override
    public void super_burn(float damage) {
        controller.getEntity().getHandle(NMSEntityHook_unused.class)
                .super_burn(damage);
    }

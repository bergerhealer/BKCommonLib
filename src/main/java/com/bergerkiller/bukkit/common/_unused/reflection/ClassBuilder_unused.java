 ...
 */
public class ClassBuilder_unused {
    ...
    private final Map<Signature, CallbackSignature_unused> callbackSignatures = new HashMap<Signature, CallbackSignature_unused>();
    private final List<Class<?>> callbackClasses;

    @SuppressWarnings("rawtypes")
    public ClassBuilder_unused(Class<?> superclass, Class... callbackClasses) {
        this(superclass, Arrays.asList((Class<?>[]) callbackClasses));
    }

    public ClassBuilder_unused(Class<?> superclass, Collection<Class<?>> callbackClasses) {
        ...
        try {
            ...
            CallbackSignature_unused callback;
                ...
                for (Method method : interfaceClass.getDeclaredMethods()) {
                    ...
                    if (name.startsWith(SUPER_PREFIX)) {
                        ...
                        try {
                            ...
                            callback = new SuperCallbackSignature_unused(superSig);
                        ...
                        for (Class<?> callBackClass : callbackClasses) {
                            if (interfaceClass.isAssignableFrom(callBackClass)) {
                                callback = new ProxyCallbackSignature_unused(callBackClass.getDeclaredMethod(method.getName(), method.getParameterTypes()));
                                break;
                            }
                        }
                    }
    ...
    private void addCallback(Method superMethod, CallbackSignature_unused callback) {
    ...
    private final class CallbackMethodInterceptor implements MethodInterceptor {
        ...
        public void createAllCallbacks(Object instance) {
            for (Map.Entry<Signature, Object> callbackEntry : this.callbackMethods.entrySet()) {
                ...
                if (callback instanceof CallbackSignature_unused) {
        ...
        public Object intercept(Object instance, Method method, Object[] args, MethodProxy proxy) throws Throwable {
            ...
            if (callback instanceof CallbackMethod_unused) {
                // Execute the method
                return ((CallbackMethod_unused) callback).invoke(instance, args);
            } else {
        ...
        private CallbackMethod_unused createCallback(Object callbackSignature, Object instance) {
            return ((CallbackSignature_unused) callbackSignature).createCallback(instance, callbackInstancesBuffer);
        }
    }
}

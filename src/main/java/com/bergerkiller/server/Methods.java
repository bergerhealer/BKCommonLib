package com.bergerkiller.server;
...
public class Methods {
	...
	public static int ItemStack_TypeId(Object itemstack) {
		//TODO: Broken!!!!
		return 0;
		//return ((ItemStack) itemstack).c
	}
	...
	public static boolean PlayerChunkQueue_Contains(Player player, int cx, int cz) {
		throw new RuntimeException("BROKEN!!!");
		/*
        List<?> chunkQue = NMSEntityPlayer.chunkQueue.get(Conversion.toEntityHandle.convert(player));
    ...
    public static boolean setBlock(org.bukkit.Chunk chunk, int x, int y, int z, Material type, int data) {
        ...
        if (result) {
            result = NMSChunk.setBlock(Conversion.toChunkHandle.convert(chunk), x, y, z, typeBlock, data);
...
//			world.notifyAndUpdatePhysics(pos, chunk, typeBlock);
    ...
    public static IBlockData getBlockData(org.bukkit.Chunk chunk, int x, int y, int z) {
        return NMSChunk.getData(CommonNMS.getNative(chunk), x, y, z);
    }
}
